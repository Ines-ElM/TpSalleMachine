...
public class MachineService extends UnicastRemoteObject implements IDao<Machine> {
    ...
    @Override
    public List<Machine> findMachine(int ids) throws RemoteException {
          Session session = null;
            Transaction tx = null;
            List<Machine> machines = null;
            try {
                ...
                Query M = session.createQuery("select m from Machine m where m.salle.id=:ids") ;
                M.setParameter("ids", ids);
                machines = M.list();
            } catch (HibernateException ex) {
                 if (tx != null) {
                     tx.rollback();
